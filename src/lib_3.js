...
class App 
{
    ...
    drawBoundary()
    {
        this.createShape([[[0, 0], [0, this._ctx.canvas.height], [this._ctx.canvas.width, this._ctx.canvas.height], [this._ctx.canvas.width, 0], [0, 0]]])
    }
...
class Shape
{
    ...
    draw(ctx, force = false, zero = null, scale = null)
    {
    ...
    isShapeCanBeSeen(ctx, zero, scale)
    {
        function inside(point, vs, zero, scale) {
        ...
        for(var x = 0; x < 2; x++)
        {
            for(var y = 0; y < 2; y++)
            {
                let check = inside([this._drawBox[0][x] * scale[0] + zero[0], this._drawBox[1][y] * scale[1] + zero[1]], mapBoundary, zero, scale)
    ...
    isPointInsideShape(point, zero, scale)
    {
        // OuterRingIsCW is a variable to contain the first polygon is it a CW or CCW so the Holes must be the other way
        // With this variable we can accept both GeoJSON format and inversed GeoJSON format
        let outerRingIsCW = null
        ...
        this._polygons.forEach((polygon) => {
            ...
            if(polygon._isClockwise === outerRingIsCW)
            {
                let inside = polygon.isPointInsidePolygon(point, zero, scale)
...
class Polygon
{
    ...
    isPointInsidePolygon(point, zero, scale)
    {
        // ray-casting algorithm based on
        // https://wrf.ecse.rpi.edu/Research/Short_Notes/pnpoly.html/pnpoly.html
        ...
        var inside = false;
        for (var i = 0, j = vs.length - 1; i < vs.length; j = i++) {
            var xi = vs[i][0] * scale[0] + zero[0], yi = vs[i][1] * scale[1] + zero[1];
            var xj = vs[j][0] * scale[0] + zero[0], yj = vs[j][1] * scale[1] + zero[1];
        ...
        this._holes.forEach((hole) => {
            let out = !hole.isPointInsidePolygon(point, zero, scale)
...
class Layer
{
    ...
    draw(ctx, forceUpdate = false, zero = null, scale)
    {
        ...
        this._shapes.forEach((shape) => {
            shape.draw(ctx, forceUpdate, zero, scale)
        })
    }

    // A function to check if the point is inside on of the shape(s) and then return the shape accordingly
    click(point, zero, scale)
    {
        let clickedShape = null

        this._shapes.forEach((shape) => {
            let inside = shape.isPointInsideShape(point, zero, scale)
            if(inside)
            {
                shape.click()
                clickedShape = shape
